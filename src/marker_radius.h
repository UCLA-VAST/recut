...
//========================== 3D ==========================
template<class T, class TMarker> double markerRadius(T* &inimg1d, int64_t* sz, TMarker & marker, double thresh, int method = DEFAULT_MARKER_RADIUS_METHOD)
{
    if (sz[2] == 1)
...
template<class T, class TMarker> double markerRadius_accurate(T* &inimg1d, int64_t* sz, TMarker & marker, double thresh)
{
...
template<class T, class TMarker> double markerRadius_fast(T* &inimg1d, int64_t* sz, TMarker & marker, double thresh)
{
	...
	for(ir = 0; ir <= max_r; ir++)
	{
		...
		double r2 = ir;
		for(int64_t k = r1 ; k < r2; k++)
		{
			for(int64_t j = r1; j < r2; j++)
			{
...
template<class T, class TMarker> double markerRadius_hanchuan_XY(T* &inimg1d, int64_t* sz, TMarker & marker, double thresh)
{
	long sz0 = sz[0];
	long sz01 = sz[0] * sz[1];
    double max_r = sz[0]/2;
        ...
        for (dz= zlower; dz <= zupper; ++dz)
            for (dy= -ir; dy <= +ir; ++dy)
                for (dx= -ir; dx <= +ir; ++dx)
                {
                    ...
                    if (r>ir-1 && r<=ir)
                    {
                        int64_t i = marker.x+dx;   if (i<0 || i>=sz[0]) goto end1;
                        int64_t j = marker.y+dy;   if (j<0 || j>=sz[1]) goto end1;
                        int64_t k = marker.z+dz;   if (k<0 || k>=sz[2]) goto end1;
...
template<class T, class TMarker> double markerRadius_hanchuan(T* &inimg1d, int64_t* sz, TMarker & marker, double thresh)
{
	long sz0 = sz[0];
	long sz01 = sz[0] * sz[1];
    double max_r = sz[0]/2;
        ...
        for (dz= zlower; dz <= zupper; ++dz)
            for (dy= -ir; dy <= +ir; ++dy)
                for (dx= -ir; dx <= +ir; ++dx)
                {
                    ...
                    if (r>ir-1 && r<=ir)
                    {
                        int64_t i = marker.x+dx;   if (i<0 || i>=sz[0]) goto end2;
                        int64_t j = marker.y+dy;   if (j<0 || j>=sz[1]) goto end2;
                        int64_t k = marker.z+dz;   if (k<0 || k>=sz[2]) goto end2;
...
template<class T> double markerRadius(T* &inimg1d, int64_t* sz, int64_t x, int64_t y , int64_t z, double thresh, int method = DEFAULT_MARKER_RADIUS_METHOD)
{
	MyMarker marker(x,y,z);
	return markerRadius(inimg1d, sz, marker, thresh, method);
}
//========================== 2D ==========================
template<class T, class TMarker> double markerRadiusXY(T* &inimg1d, int64_t* sz, TMarker & marker, double thresh)
{
	return markerRadius_hanchuan_XY(inimg1d, sz, marker, thresh);
}
