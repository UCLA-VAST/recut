...
void RecutCommandLineArgs::PrintUsage()
{
    cout<< "Usage : ./recut <image_root_dir> <channel> [-inmarker <marker_dir>] [-outswc <swc_file>] "
            ...
            "[-bkg-thresh <int>] [-fg-percent <double>] [-gsdt] [-cnn-type <int>] [-length-thresh <double>] [-allow_gap] [-pl [<int>]]" << endl;
    cout << endl;
    cout << "-inmarker           [-im] input marker file directory all marker files represent known soma locations" << endl;
    ...
    cout << "-bg-thresh          [-bt] background threshold value used in GSDT and tree construction when no target marker" << endl;
    cout << "-max                      set the max voxel value manually such that no extra traversal of image is done" << endl;
    cout << "-min                      set the min voxel value manually such that no extra traversal of image is done" << endl;
    cout << "-fg-percent         [-fp] default 0.01, percent of voxels to be considered foreground. overrides -bg-thresh" << endl;
    ...
    cout << "-allow_gap          [-ag] accept one background point between foreground during tree construction when only one marker" << endl;
    cout << "-prune              [-pr] prune 0 false, 1 true; defaults to 1 (automatically prunes)" << endl;
    cout << "-parallel           [-pl] parallelize with specified # of threads; defaults to max threads, otherwise -pl THREAD-NUM is used" << endl;
    cout << "-interval_size      [-is] interval size for in memory size to parallelize ; defaults to interval cubes of edge length 1024" << endl;
    cout << "-block_size         [-bs] block size for thread level parallelize option; defaults to cubes of edge length 32" << endl;
    cout << "-restart            [-rs] enforce parallel restarts; default is false, if true with no number passed restart factor defaults to 4" << endl;
    cout << endl;
}
