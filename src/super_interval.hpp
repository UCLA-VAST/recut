...
public:
    ...
    for (auto interval_num = 0; interval_num < nintervals; interval_num++) {
      ...
      try {
        intervals_.rbegin()->reset(new Interval(
            offset, interval_size, interval_num, default_interval, mmap_));
      } catch (...) {
        cout << "********* Failed to create new Interval. quit "
                "fastmarching_tree()."
             << '\n';
      }

#ifdef DEBUG
      // cout<< "Created interval # "<< interval_num << '\n';
      ...
      // cout << "iblock " << iblock << " jblock " << jblock << " kblock " <<
      // kblock << " offset " << offset << " interval " << interval << '\n';
#endif
    }
  }
  ...
  inline Interval *GetInterval(size_t idx) {
    assertm(
        idx < nintervals_,
        "Requested interval can not exceed total contained in SuperInterval");
