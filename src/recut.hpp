...
Recut<image_t>::process_marker_dir(const GridCoord grid_offsets,
  ...
  rng::for_each(
    ...
    auto roots =
        inmarkers | rng::views::transform([](auto marker) {
          return std::pair{ones() + GridCoord(marker.x / DOWNSAMPLE_MARKER,
                                              marker.y / DOWNSAMPLE_MARKER,
                                              marker.z),
    ...
    return roots |
           rng::views::remove_if([this, &local_bbox](auto coord_radius) {
...
#ifdef FULL_PRINT
    auto nb_block_coord =
        id_to_coord(nb_block_id, this->interval_block_lengths);
...
void Recut<image_t>::check_ghost_update(VID_t interval_id, VID_t block_id,
    ...
    if (dst_offsets[0] == 0) {
      if (dst_coord[0] > 0) { // protect from image out of bounds
        ...
        if ((nb >= 0) &&
            (nb < interval_block_size)) // within valid block bounds
...
void Recut<image_t>::connected_tile(
    ...
    while (!(connected_fifo.empty())) {
      ...
      auto valids =
          ...
          // within image?
          rng::views::remove_if(
              [this, &found_adjacent_invalid](auto coord_img) {
...
void Recut<image_t>::radius_tile(const image_t *tile, VID_t interval_id,
    ...
    while (!(fifo.empty())) {
      ...
      if (msg_vertex->radius < 1) {
        ...
        err << " handle radius has " << +(radius_handle.get(*msg_ind))
            << " msg? " << msg_vertex->unselected() << "surf? "
            << msg_vertex->surface() << ' ' << msg_vertex->offsets << '\n';
        cout << err.str() << '\n';
...
void Recut<image_t>::prune_tile(const image_t *tile, VID_t interval_id,
    ...
    while (!(fifo.empty())) {
      ...
      cout << '\n'
           << coord_to_str(msg_coord) << " interval " << interval_id
           << " block " << bbox.min() << " label " << current->label()
...
#endif
    auto timer = high_resolution_timer();
    assertm(foreground_percent >= 0.,
            "foreground_percent must be 0 or positive");
    assertm(foreground_percent <= 100.,
            "foreground_percent must be 100 or less");
...
Recut<image_t>::update(std::string stage, Container &fifo,
    ...
    auto visited_intervals =
        std::make_unique<bool[]>(grid_interval_size, false);
#endif
...
std::vector<std::pair<GridCoord, uint8_t>> Recut<image_t>::initialize() {
    ...
    auto max_len_after_off =
        coord_sub(input_image_lengths, this->image_offsets);
    ...
    if (this->params->convert_only_) {
      ...
      this->interval_lengths[0] =
          std::min(default_size, this->image_lengths[0]);
      this->interval_lengths[1] =
          std::min(default_size, this->image_lengths[1]);
      this->interval_lengths[2] =
          std::min(default_size, this->image_lengths[2]);
    }
    ...
    if (this->params->force_regenerate_image) {
      ...
      assertm(this->image_lengths[0] == this->image_lengths[1],
              "change create_image implementation to support non-cube images");
      auto selected = create_image(
          this->params->tcase, this->generated_image, this->image_lengths[0],
          this->params->selected, this->params->root_vid);
      ...
      return {std::pair{
          id_to_coord(this->params->root_vid, this->image_lengths), 1}};
...
template <class image_t> void Recut<image_t>::adjust_parent() {
    ...
    auto all_valid = [](const auto &flags_handle, const auto &parents_handle,
                        const auto &radius_handle, const auto &ind) {
      return is_valid(flags_handle, ind);
    };
...
template <class image_t> void Recut<image_t>::operator()() {
    ...
    rng::for_each(components, [this, &counter, float_grid, output_topology,
      ...
      rng::for_each(component_roots, [this, &file](const auto &component_root) {
        print_swc_line(component_root.first, /*root*/ true,
                       component_root.second, zeros_off(), this->image_lengths,
      ...
      rng::for_each(filtered_spheres, [this, &file, component,
        ...
        assertm(component->tree().isValueOn(coord),
                "component value must be on");
