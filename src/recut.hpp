    ...
    void Recut<image_t>::integrate_updated_ghost(VID_t interval_id, VID_t block_id,
      ...
      template <class image_t>
        int Recut<image_t>::get_bkg_threshold(const image_t *tile, VID_t interval_vertex_size,
          ...
          VID_t bkg_count;
          for (image_t local_bkg_thresh = 0; local_bkg_thresh <= std::numeric_limits<image_t>::max(); local_bkg_thresh++) {
      ...
      template <class image_t>
        void Recut<image_t>::load_tile(VID_t interval_id, mcp3d::MImage &mcp3d_tile) {
          ...
          assertm(!(this->params->force_regenerate_image),
              "If USE_MCP3D macro is set, this->params->force_regenerate_image must be set "
              "to False");
        ...
        Recut<image_t>::get_tile_thresholds(mcp3d::MImage &mcp3d_tile) {
          ...
          if (params->foreground_percent() >= 0) {
            tile_thresholds->bkg_thresh = get_bkg_threshold(
                mcp3d_tile.Volume<image_t>(0), interval_vertex_size,
      ...
      template <class image_t> std::vector<VID_t> Recut<image_t>::initialize() {
        ...
        for (int i = 0; i < 3; i++) {
          ...
          if (args->image_extents[i]) {
            ...
            // image_extents is set to grid_size for force_regenerate_image option, otherwise
