...
def is_match(euc_dist: List[Tuple[int, ...]], distance_threshold):
    """
    ...
    for soma_inf in euc_dist:
        ...
        is_match_each = [1 if d <= distance_threshold else 0 for d in soma_inf_lst]
...
def plot_his_cdf(inference_coord_radii_list, FP_coord_radii_list, distance_threshold, current_time, path_result):
    """
...
def main():
    ...
    parser.add_argument('labeled-path', help="input path for labeled soma location")
    parser.add_argument('inferenced-path', help="input path for inferenced soma location")
