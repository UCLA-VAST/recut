...
#include <chrono>
#include <omp.h>
#include <predef/include/boost/predef.h>
#include <type_traits>
...
static_assert(
    std::is_nothrow_copy_constructible<mcp3d::MCPInvalidArgument>::value,
...
#define MCP3D_ASSERT(expr)                                                     \
  {                                                                            \
    if (expr)                                                                  \
      ...
      throw ::mcp3d::MCPAssertionError(#expr, __FILE__, __LINE__, __func__);   \
  }

#define MCP3D_IMAGE_FORMAT_UNSUPPORTED(arg)                                    \
  {                                                                            \
    throw ::mcp3d::MCPImageFormatUnsupportedError(arg, __FILE__, __LINE__,     \
                                                  __func__);                   \
  }

#define MCP3D_PRINT_NESTED_EXCEPTION                                           \
  {                                                                            \
    ::mcp3d::PrintNestedException(::std::current_exception(), __FILE__,        \
                                  __LINE__, __func__);                         \
  }
