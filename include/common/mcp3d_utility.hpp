...
public:
    ...
    // for easy testing
    explicit DFSDirectoryWalker(const std::string& root_dir, bool sort_alpha = false);
    // terminal directory:
    // (1) empty or (2) contain only files and no other directory or
    // (3) has subdirectories but all of them already expanded
    // terminals are reached in DFS order, with tree structure based on tree_stack_
    // tree nodes are only expandable at directories, which are always placed at the front
    // of each tree depth. if a tree level has all its terminal directories
    // visited, the next ExpandNextTerminal() place all files as well as directories
    // at same depth in files_in_terminal_dir. and view the parent dir of current
    // level as terminal directory
...
private:
    ...
    std::vector<int> tree_path_to_frontier_dir_, terminal_dir_counts_, terminal_entry_counts_;
...
MCP3D_EXPORT
std::vector<std::string> DirsInDir(const std::string &directory_,
                                   bool full_path = true,
                                   bool sort_files = true,
                                   const std::vector<std::string> &incl = std::vector<std::string>(),
                                   int n = -1,
...
bool IsFile(const boost::filesystem::path& file_path);

bool IsDir(const std::string &dir_path);

bool IsEmptyDir(const std::string &dir_path);

boost::filesystem::path ResolveSymlink(const std::string &path_);
...
/// refer here: http://www.boost.org/doc/libs/1_53_0/libs/filesystem/doc/reference.html#Path-decomposition-table
...
SplitBaseName(const std::string &file_path);

/// return the pair: root dir and remainder of path
std::pair<std::string, std::string>
SplitFirstDir(const std::string &file_path);
...
/// list_directories_firs = true and reverse = false will return directories last
#if BOOST_OS_LINUX
std::vector<std::string> ListDir(const std::string &dir_path, bool sort_alpha = false,
                                 bool list_directories_first = true, bool revert = false);
...
/// if path starts with component (this is affirmed when leading '/' is collapsed
/// in both), return the remainder path after leading component is discarded,
/// leading and trailing '/' not kept in output.
/// otherwise return original path unmodified
/// e.g. /foo/bar, /foo -> bar
...
std::string JoinPath(const Ts &...args);
...
template<typename T>
std::vector<int> To5D(const std::vector<T> &dims_);
...
std::vector<T1> AddSeq(const std::vector<T2> &lhs,
...
template<typename Container>
bool AllPositive(const Container &container);

template<typename Container>
bool AllNonNegative(const Container &container);

template<typename Container>
bool AllNonZero(const Container &container);

template<typename Container>
bool AllZeros(const Container &container);

template<typename Container>
bool AnyPositive(const Container &container);

template <typename Container>
bool AnyNegative(const Container& container);

template<typename Container>
bool AnyZeros(const Container &container);
...
inline std::string SysCmdResult(const std::string& cmd, const std::string& mode = "")
{ return SysCmdResult(cmd.c_str()); }

template <typename T = long>
T IntPow(int base, int exp);
...
template <typename ...Ts>
std::string mcp3d::JoinPath(const Ts&...args)
{
...
template <typename T>
std::string mcp3d::JoinVector(const std::vector<T>& v,
                              const std::string& d, bool brackets)
{
...
template<typename Container>
Container mcp3d::Minimum(const Container &container0, const Container &container1)
{
...
T mcp3d::ReduceProdSeq(const Container& container)
{
    ...
    if (sizeof(T) < sizeof(element_type) ||
        (std::is_floating_point<element_type>() && std::is_integral<element_type>()))
        std::cout << "warning: output type " << typeid(T).name()
                  << " is narrower than input type " << typeid(element_type).name() << std::endl;
    T acc(1);
...
template<typename T>
std::vector<T> mcp3d::operator+(const std::vector<T> &lhs, const std::vector<T> &rhs)
{
...
{
    static_assert(std::is_arithmetic<T1>() && std::is_arithmetic<T2>() &&
                  std::is_arithmetic<T3>(), "container elements must be arithmetic type");
    MCP3D_ASSERT(numerator.size() == denominator.size())
   if (! AnyZeros<std::vector<T3>>(denominator))
       MCP3D_RUNTIME_ERROR("division by 0")
    std::vector<T1> output;
    for (size_t i = 0; i < numerator.size(); ++i)
        output.push_back(static_cast<T1>(numerator[i]) / static_cast<T1>(denominator[i]));
    return output;
}
...
{
    MCP3D_ASSERT (!container.empty())
    static_assert(std::is_integral<typename Container::value_type>(),
...
template<typename Container>
bool mcp3d::AllZeros(const Container &container)
{
    MCP3D_ASSERT (!container.empty())
    static_assert(std::is_integral<typename Container::value_type>(),
                  "container elements must be integer for equality evaluation with zero");
...
template<typename Container>
bool mcp3d::AnyZeros(const Container &container)
{
    MCP3D_ASSERT (!container.empty())
    static_assert(std::is_integral<typename Container::value_type>(),
                  "container elements must be integer for equality evaluation with zero");
...
template <typename T>
T mcp3d::IntPow(int base, int exp)
{
    if (exp < 0 || base < 0)
